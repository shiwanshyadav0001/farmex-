...
function CropRecommendation() {
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      const response = await axios.post(`${API}/crop/recommend`, { ...formData, language });
      ...
      <div className="mx-auto max-w-7xl">
        ...
        <div className="grid grid-cols-1 gap-8 lg:grid-cols-[0.95fr_1.05fr]">
          <div className="card fade-in">
            <h2 className="mb-6 text-2xl font-bold text-gray-800 dark:text-gray-200 dark:text-gray-100">{t('Farm Details')}</h2>
            <form onSubmit={handleSubmit} className="space-y-5">
              ...
              <div>
                <label className="mb-2 block text-sm font-medium text-gray-700 dark:text-gray-300 dark:text-gray-200">{t('Season')}</label>
                <select
              ...
              <div>
                <label className="mb-2 block text-sm font-medium text-gray-700 dark:text-gray-300 dark:text-gray-200">{t('Available Area (acres)')}</label>
                <input
                  type="number"
                  step="0.1"
                  value={formData.area}
              ...
              <div>
                <label className="mb-2 block text-sm font-medium text-gray-700 dark:text-gray-300 dark:text-gray-200">{t('Preferences (Optional)')}</label>
                <textarea
          ...
          <div className="card fade-in" data-testid="crop-recommendations-display">
            <h2 className="mb-6 text-2xl font-bold text-gray-800 dark:text-gray-200 dark:text-gray-100">{t('AI Recommendations')}</h2>
            {loading ? (
              <div className="flex flex-col items-center justify-center py-16">
                <Loader className="mb-4 h-16 w-16 animate-spin text-green-600" />
                <p className="text-gray-600 dark:text-gray-400 dark:text-gray-300">{t('Analyzing your farm conditions...')}</p>
              </div>
            ) : recommendation ? (
              <div className="space-y-6">
                <div className="rounded-xl border border-green-200 dark:border-green-800/50 bg-green-50 dark:bg-green-900/20 p-4">
                  ...
                  <p className="text-sm text-gray-600 dark:text-gray-400 dark:text-gray-300">{t('Stable result for the same input, based on weather and soil conditions')}</p>
                </div>

                {cropCards.length > 0 ? (
                  <div className="space-y-4">
                    {cropCards.map((crop, index) => (
                      <div key={`${crop.name}-${index}`} className="overflow-hidden rounded-[28px] border border-gray-200 dark:border-slate-700 bg-white dark:bg-slate-900 p-5 shadow-[0_18px_55px_rgba(15,23,42,0.08)]">
                        <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
                          <div className="max-w-xl">
                            <div className="inline-flex items-center gap-2 rounded-full bg-emerald-50 dark:bg-emerald-900/20 px-3 py-1 text-xs font-semibold uppercase tracking-[0.18em] text-emerald-700">
                              <Leaf className="h-3.5 w-3.5" />
                              {index === 0 ? 'Best Match' : `Option ${index + 1}`}
                            </div>
                            <h3 className="mt-3 text-2xl font-black text-slate-900 dark:text-slate-50">{crop.name}</h3>
                          ...
                          <div className="grid min-w-[220px] grid-cols-2 gap-3">
                            <div className="rounded-2xl border border-slate-200 dark:border-slate-700 bg-slate-50 dark:bg-slate-800 px-4 py-3">
                              <div className="text-[11px] font-semibold uppercase tracking-[0.18em] text-slate-500 dark:text-slate-400">Suitability</div>
                              <div className="mt-2 text-2xl font-black text-slate-900 dark:text-slate-50">{crop.score}%</div>
                            </div>
                            <div className="rounded-2xl border border-slate-200 dark:border-slate-700 bg-slate-50 dark:bg-slate-800 px-4 py-3">
                              <div className="text-[11px] font-semibold uppercase tracking-[0.18em] text-slate-500 dark:text-slate-400">Market Fit</div>
                              <div className="mt-2 text-base font-bold text-slate-900 dark:text-slate-50">{crop.marketFit}</div>
                            </div>
                          </div>
                        </div>
                        ...
                        <div className="mt-5 grid gap-3 md:grid-cols-3">
                          ...
                          <div className="rounded-2xl border border-slate-200 dark:border-slate-700 bg-slate-50 dark:bg-slate-800 px-4 py-4">
                            ...
                            <div className="mt-2 text-sm font-semibold text-slate-800 dark:text-slate-200">{crop.waterNeed}</div>
                          </div>
                          <div className="rounded-2xl border border-slate-200 dark:border-slate-700 bg-slate-50 dark:bg-slate-800 px-4 py-4">
                            <div className="flex items-center gap-2 text-[11px] font-semibold uppercase tracking-[0.18em] text-slate-500 dark:text-slate-400">
                ...
                {recommendation.weather_context && (
                  <div className="rounded-[24px] border border-blue-200 dark:border-blue-800/50 bg-blue-50 dark:bg-blue-900/20 p-5">
                    <h3 className="mb-4 font-semibold text-gray-800 dark:text-gray-200">{t('Current Weather Context')}</h3>
                    <div className="grid gap-3 md:grid-cols-4 text-sm">
                      <div className="rounded-2xl border border-blue-100 dark:border-blue-800/50 bg-white dark:bg-slate-900 px-4 py-3">
                        <div className="flex items-center gap-2 text-[11px] font-semibold uppercase tracking-[0.18em] text-slate-500 dark:text-slate-400">
                          <Thermometer className="h-3.5 w-3.5" />
                          Temperature
                        </div>
                        <div className="mt-2 text-lg font-bold text-slate-900 dark:text-slate-50">{recommendation.weather_context.temp}°C</div>
                      </div>
                      <div className="rounded-2xl border border-blue-100 dark:border-blue-800/50 bg-white dark:bg-slate-900 px-4 py-3">
                        <div className="flex items-center gap-2 text-[11px] font-semibold uppercase tracking-[0.18em] text-slate-500 dark:text-slate-400">
                          <Droplets className="h-3.5 w-3.5" />
                        ...
                        <div className="mt-2 text-lg font-bold text-slate-900 dark:text-slate-50">{recommendation.weather_context.humidity}%</div>
                      </div>
                      <div className="rounded-2xl border border-blue-100 dark:border-blue-800/50 bg-white dark:bg-slate-900 px-4 py-3">
                        <div className="flex items-center gap-2 text-[11px] font-semibold uppercase tracking-[0.18em] text-slate-500 dark:text-slate-400">
                          <Leaf className="h-3.5 w-3.5" />
                        ...
                        <div className="mt-2 text-lg font-bold text-slate-900 dark:text-slate-50">{formData.soil_type}</div>
                      </div>
                      <div className="rounded-2xl border border-blue-100 dark:border-blue-800/50 bg-white dark:bg-slate-900 px-4 py-3">
                        <div className="flex items-center gap-2 text-[11px] font-semibold uppercase tracking-[0.18em] text-slate-500 dark:text-slate-400">
                          <Sprout className="h-3.5 w-3.5" />
                        ...
                        <div className="mt-2 text-lg font-bold text-slate-900 dark:text-slate-50">{formData.season}</div>
                      </div>
                    </div>
                  </div>
                )}
